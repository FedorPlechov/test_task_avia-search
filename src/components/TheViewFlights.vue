<template>
  <div class="container__main">
    <TheFilterForm @sort="setSort($event)"
    ...
    />
    <TheListOfFlights :flights="flights"
                      :sort="sort"
...
export default {
  ...
  computed: {
    allAirlinesBestPrice() {
      const arr = [];
      const companies = this.flights.map((el) => {
          return {
            carrier: el.carrier,
            price: el.price,
            numbersOfConnections: (+el.flyTo.hasConnectionFlight) + (+el.flyBack.hasConnectionFlight),
            duration: el.flyTo.duration + el.flyBack.duration
          };
        }
      ).filter((el) => {
        return (this.priceFilter.start < el.price && this.priceFilter.end > el.price && this.filterConnectionsCompanies(el));
      });
      return (this.sortCompanies(companies).filter((item) => this.itemCheckBestPriceAllCompanies(item, arr)));
    }
  },
  methods: {
    console(a) {
    ...
    filterConnectionsCompanies(el) {
      return this.numbersOfConnections.length === 0 ? true :
        this.numbersOfConnections.length === 1 ? el.numbersOfConnections === +this.numbersOfConnections[0] :
          el.numbersOfConnections === +this.numbersOfConnections[0] || el.numbersOfConnections === +this.numbersOfConnections[1];
    },
